...
A rendered, slide-ready version is at `flow-diagram.png` (in this `docs/` folder).
The same diagram as editable Mermaid:

```mermaid
flowchart LR
    subgraph Browser["🖥️ Browser (UI)"]
        U["1 · Upload<br/>drag-drop · paste · PDF"]
        P["Preview + pick models"]
    end
    subgraph Server["⚙️ Server — Next.js API"]
        API["2 · /api/extract<br/>keys stay server-side"]
        ABS["3 · Model abstraction<br/>one interface"]
        VAL["4 · Validate JSON<br/>Zod schema"]
    end
    subgraph Models["☁️ AI models"]
        M1["Gemini (free)"]
        M2["OpenRouter (free)"]
        M3["GPT-4o"]
        M4["Claude"]
    ...
    subgraph Results["📊 Results"]
        CMP["5 · Compare<br/>JSON · latency · tokens"]
        SCORE["Score vs ground truth<br/>accuracy % + diff"]
    end
    GT["📁 samples/*.json<br/>ground truth"]

    U --> P --> API --> ABS
    ABS --> M1 & M2 & M3 & M4
    M1 & M2 & M3 & M4 --> VAL --> CMP --> SCORE
    GT -.-> SCORE
```
